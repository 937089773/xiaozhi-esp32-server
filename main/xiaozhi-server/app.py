...
async def main():
    ...
    mqtt_task = None
    mqtt_transport = None
    mqtt_transport_config = config.get("mqtt_transport", {})
    if mqtt_transport_config.get("enabled", False):
        mqtt_transport = MqttTransport(mqtt_transport_config, app_config=config, server=ws_server)
        mqtt_task = asyncio.create_task(mqtt_transport.start())
    # 启动 Simple http 服务器
